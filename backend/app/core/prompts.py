...
{{
  "position_title": "职位名称",
  "company": "公司名称",
  "domain": "岗位所属领域，如: AI应用开发/后端开发/前端开发/数据分析/计算机视觉/嵌入式/游戏开发/生物医药等",
  "domain_keywords": ["该领域的核心关键词，用于检索时引导匹配方向"],
  "irrelevant_domains": ["与该岗位明确无关的领域，如AI应用开发岗位则排除: 计算机视觉/目标检测/机器人/自动驾驶/嵌入式/游戏开发等"],
  "core_requirements": [
...
# ============================================================
# LLM 相关性把关 — 检索后过滤跨领域误匹配
# ============================================================

RELEVANCE_FILTER_SYSTEM = """你是一位严格的招聘匹配专家。你的任务是判断候选经历（项目/工作/实习）是否与目标JD的方向一致。

## 判断标准
- **相关**: 经历的技术栈、业务领域、解决的问题与JD方向一致（如JD是"AI应用开发"→ LLM应用/RAG/Agent相关）
- **不相关**: 经历属于与JD明确不同的方向（如JD是"AI应用开发"→ 计算机视觉YOLO/目标检测/图像处理不相关）

## 严格要求
- 方向不一致就是不一致，不要因为"都用Python"就觉得相关
- 如果不确定，保守判断为不相关"""

RELEVANCE_FILTER_USER = """判断以下候选经历（项目/工作/实习）是否与目标JD方向一致。
...
3. **量化呈现**: 把素材中已有的数字（模块数量、性能、规模、覆盖率、耗时等）全部保留并放在要点最前面；不要凭空编造百分比或数据
4. **设计思路优先**: 项目描述以「为什么这么设计」为主体（设计动机 → 核心方案 → 为什么选它而非其他），技术选型只作设计支撑、次位呈现，不要堆砌技术名词
5. **去冗余**: 删除空洞的修饰词和无意义的套话
...
- 技术栈/工具：技术1 / 技术2（次位，写清素材中有的硬技能）
- 项目描述：一句话交代「为什么这么设计」——设计动机/核心方案/为什么选它而非其他（STAR中的S，须素材能看出，不脑补）
- 核心职责与成果：（2-4个要点，动词开头，用「对比 + 数字」量化，只使用素材中已有的数字）
...
**项目名称（一句话概括项目核心）｜ 时间**
- 技术栈/工具：技术1 / 技术2 / 技术3（次位，写清硬技能）
- 项目描述：一句话交代「为什么这么设计」——设计动机/核心方案/为什么选它而非其他，为解决XXX问题而搭建的XX系统（须素材能看出，不脑补）。
- 核心职责与成果：
  - 动词开头，量化优先：用「对比 + 数字」句式（采用 A 策略，相比 B 方案，更契合 C 场景，X 提升 Y%）
  - 动词开头，要点2
  - 动词开头，要点3
...
2. **禁止虚构任何信息**：包括但不限于 — 不存在的公司名、职位名、项目名、技术栈、时间、数字、成果、架构设计、模块划分。
...
2. **润色要求**:
   ...
   - **量化优先**：素材中已有的数字（模块数量、性能、规模、覆盖率、耗时等）必须全部保留，并放在要点的最前面
   - **突出设计思路**：项目描述要交代「怎么设计的、为什么这样设计」，而不是只罗列「做了什么」
   - 删除空洞修饰词和套话
...
描述项目时，**设计思路是主体**（回答"为什么这么设计"），技术实现次之，成果必须量化。

1. **设计思路优先（主体）**:
   - 先用一句话讲清设计动机：解决什么痛点、现有方案为什么不够好
   - 交代核心设计决策与架构：拆分了哪些模块、采用了什么方案，以及「为什么选这个方案而非其他」
   - 技术选型是设计的支撑：一句话点明「选 X 是为了解决 Y」，不要堆砌技术名词
   - ⚠️ 设计和架构必须能从素材中看出，不得脑补不存在的设计

2. **技术次之**:
   - 技术栈/工具列表放在设计思路之后，只列素材中真实用到的硬技能

3. **成果必须量化（回答"这个设计带来了多大价值"）**:
   - 用「对比 + 数字」的方式表达，把素材中的数字、指标全部提取并前置
   - 理想句式：**「采用 A 策略/模型，相比 B 方案，更契合 C 场景，X 提升 Y%」**（例：采用混合检索 + 相关性把关策略，相比单纯相似度检索，更契合 JD 简历重组场景，检索准确率提升 XX%）
   - ⚠️ 只能使用素材中已有的数字；素材没有数字时用「功能覆盖范围」等真实描述替代，绝不编造数字"""
...
从素材中提取与JD最相关的2-4个项目。每条严格按以下模板:
  ...
  技术栈/工具：保留素材中列出的技术
  项目描述：一句话交代「为什么这么设计」——解决什么痛点、采用了什么方案、为什么选它而非其他方案；技术选型只作设计支撑，不堆技术名词（须素材中能看出，不得脑补）
  核心职责与成果：
    - 动词开头，量化优先，用「对比 + 数字」句式（如「采用 A 策略，相比 B 方案，更契合 C 场景，X 提升 Y%」）
    - 基于素材中的职责描述改写，保留原始数字，不要添加素材中没有的成果或数字
